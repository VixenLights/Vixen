...
Shared xModel coordinate scaling applies to both custom and circle imports. When `ScaleX` or `ScaleY` is present, the importer applies it to the corresponding generated coordinate axis before rounding. When the value is missing, invalid, or non-positive, the importer uses the same dimension-based default scale that custom imports historically used.
...
| `ScaleX` | no | xLights screen-location X scale. Apply to generated X coordinates before rounding. Missing, invalid, or non-positive values use the shared custom-model default scale. |
| `ScaleY` | no | xLights screen-location Y scale. Apply to generated Y coordinates before rounding. Missing, invalid, or non-positive values use the shared custom-model default scale. |
...
- `ScaleX` or `ScaleY` is missing, invalid, or non-positive. Use the same dimension-based default scale as custom model imports: `4` for models smaller than `100x100`, `2` for models smaller than `200x200`, otherwise `1`.
...
- Apply optional xLights `ScaleX` and `ScaleY` values to the generated centered coordinates before rounding. Missing or invalid scale uses the shared custom-model default scale. This bakes the xLights screen-location scale into Vixen's fixed light coordinates and avoids jagged small-ring imports.
