...
- [x] (2026-07-02) Milestone 4: Catalog service — `ElementTagService` with CRUD, built-in resolution, validation, and cross-node cleanup. Done — 8 new unit tests pass (`ElementTagServiceTests`, covering `ValidateAndNormalizeTagName`), full solution builds clean, full suite 99/99 (later 101/101 after the trimming fix) passes. Required pulling `VixenSystem.TagDefinitions` forward from Milestone 5 as a stub (see Decision Log) and adding an `InternalsVisibleTo` grant so the pure validation helper can be unit tested.
- [x] (2026-07-02) Milestone 5: Persistence — `SystemConfig.Tags`, XML serializers (`XmlElementTagDefinitionSerializer`, `XmlElementTagDefinitionCollectionSerializer`), `SystemConfigFilePolicy`/`XmlSystemConfigFilePolicy` wiring, per-node tag persistence in `XmlElementNodeSerializer`, and `VixenSystem.TagDefinitions` load/save/seed/cleanup wiring (`EnsureBuiltInElementTags()`, `CleanUpOrphanedElementTagAssignments()`). Done — 9 new unit tests pass (`XmlElementTagDefinitionSerializerTests`, `XmlElementTagDefinitionCollectionSerializerTests`), full suite 110/110 passes, full solution builds clean. Manual verification against the running app (built-in tag seeding, idempotent reseed, pre-existing profile load) deferred to Milestone 7 as planned.
- [x] (2026-07-02) Milestone 6: Apply project skills (`csharp-docs`, `dotnet-best-practices`, `csharp-async`, `dotnet-design-pattern-review`) and address findings. Done — `csharp-docs` review found all new public members already fully documented from Milestones 3-5. `dotnet-best-practices` found one real defect: `VixenSystem.EnsureBuiltInElementTags()`/`CleanUpOrphanedElementTagAssignments()` used space indentation (copied from an adjacent legacy method) instead of tabs; fixed to tabs per `CLAUDE.md`. String-interpolated NLog calls and non-sealed internal classes were checked against the skill's stricter generic guidance but left as-is since they match the established, repository-wide convention (verified via grep across `Vixen.Core`) — reformatting them would violate `CLAUDE.md`'s "avoid reformatting unrelated code" rule and introduce inconsistency with sibling classes. `csharp-async` found no async code introduced by this plan. `dotnet-design-pattern-review` confirmed `ElementTagService` correctly mirrors the existing `ElementNodeService` singleton pattern; the skill's console-app-oriented pattern requirements (Command Handler, DI primary constructors) don't apply to this WPF module architecture. Full suite 110/110 passes after the fix; full solution builds clean.
- [x] (2026-07-02) Milestone 7: Manual end-to-end verification against the running application and JIRA issue closeout. Done — manual verification against the running application completed. JIRA issue VIX-3933 is not being manually transitioned; the team's workflow transitions it automatically when the PR is created, so no `mcp__atlassian__transitionJiraIssue` call was made.
- [x] (2026-07-02) Milestone 8: (addendum, added 2026-07-02): Give each built-in tag a default `DisplayColor` in `BuiltInElementTags.CreateDefaults()` — `Deprecated` = red, `Prop` = blue, `Hidden` = black — instead of leaving `DisplayColor` unset as Milestone 3 originally left it. This requirement surfaced while specifying VIX-2690 (`docs\vix-2690-element-tags-workflow.md`), whose tag-color dots and color editor would otherwise have nothing to render for a never-before-colored built-in tag; the user decided a tag's default appearance belongs here, in the ticket that owns `BuiltInElementTags.cs`, not in VIX-2690. Not started — no code has been changed yet for this milestone. When implemented: update `CreateDefaults()` at `src/Vixen.Core/Sys/BuiltInElementTags.cs:49-57`; choose a hex-string `DisplayColor` format consistent with any existing string-color convention elsewhere in the codebase (investigate during implementation — none was found reused verbatim as of this addendum); add/update unit tests in `src/Vixen.Tests/Sys/BuiltInElementTagsTests.cs` asserting the three default colors; confirm `EnsureBuiltInElementTags()` is left untouched so existing profiles' already-persisted (colorless) built-in tag definitions are not retroactively recolored, only newly-seeded ones get a default.
...
- Decision: `VixenSystem.TagDefinitions` (the plain `public static List<ElementTagDefinition>`) is declared in Milestone 4, not Milestone 5 as originally planned, initialized to an empty list.
  Rationale: The plan's own validation step requires the solution to compile after every milestone, and `ElementTagService` (Milestone 4) reads/writes `VixenSystem.TagDefinitions` directly per its Plan of Work description. Declaring only the stub now — with the same shape the plan specifies for Milestone 5 — lets Milestone 4 compile and be tested in isolation. Milestone 5 does not redeclare the property; it wires it into `LoadSystemConfig()` (populate from `SystemConfig.Tags`, seed built-ins, orphan cleanup) and `SaveSystemConfigAsync()` (write back to `SystemConfig.Tags`), exactly as originally planned.
  Date/Author: 2026-07-02, Milestone 4 implementation.

- Decision: New logging calls (`EnsureBuiltInElementTags()`, `CleanUpOrphanedElementTagAssignments()`) and the new internal XML serializer classes keep string-interpolated NLog messages and non-sealed classes rather than adopting the generic `dotnet-best-practices` skill's stricter preferences (structured/templated logging, `sealed` on classes not intended for inheritance).
  Rationale: A repository-wide grep of `Logging.Info`/`Warn`/`Error` calls in `Vixen.Core` shows string interpolation is the actual, consistent codebase convention, not templated logging; the sibling internal serializer classes (`XmlOutputFilterSerializer`, `XmlPreviewSerializer`, etc.) and `ElementNodeService` are likewise none of them `sealed`. `CLAUDE.md` explicitly says "avoid reformatting unrelated code," and matching new code to the file's own established style is more valuable than conforming new code alone to a generic external checklist while every neighboring class diverges from it.
  Date/Author: 2026-07-02, Milestone 6 implementation.

- Decision: Added `<InternalsVisibleTo Include="Vixen.Tests" />` to `Vixen.Core.csproj`, not called for explicitly in the plan.
  Rationale: The plan's Milestone 4 instructions require testing `ElementTagService`'s pure validation logic via an `internal static` helper (`ValidateAndNormalizeTagName`) without touching live `VixenSystem` state, but `Vixen.Tests` had no prior access to `Vixen.Core` internals. This grant is the standard, minimal-footprint way to let a test project reach `internal` members without making the helper `public` (which would needlessly widen the public API for a detail no external caller should call directly).
  Date/Author: 2026-07-02, Milestone 4 implementation.

- Decision: The validation helper was renamed from `ValidateNewTagName` (throws only) to `ValidateAndNormalizeTagName` (throws or returns the trimmed name), and `CreateUserTag`/`RenameUserTag` now call it instead of trimming locally before validating.
  Rationale: A code-review pass caught that the original split — trim in the public method, validate in the internal helper — meant the helper's duplicate check silently depended on every caller pre-trimming input; a future caller that forgot would let `"  Outdoor  "` coexist with `"Outdoor"` even though the plan's acceptance criteria (Milestone 4) explicitly requires padded duplicates to be rejected. Folding normalization into the helper makes it the single source of truth and closes a test-coverage gap the `VixenSystem`-free testing boundary had previously hidden (trimming behavior was previously only exercised by the untestable `VixenSystem`-dependent code path).
  Date/Author: 2026-07-02, Milestone 4 implementation.

- Decision: Add Milestone 8 as an addendum to this already-completed plan, giving the three built-in tags default `DisplayColor` values (`Deprecated` = red, `Prop` = blue, `Hidden` = black) rather than leaving them unset.
  Rationale: `docs\vix-3933-element-tags.md`'s "Default built-in tag colors" resolved decision explains the full context — this requirement was discovered while specifying VIX-2690's tag-color UI, and the user decided it belongs in this ticket's `BuiltInElementTags.cs`, which this plan already delivered, rather than being bundled into VIX-2690's own scope. Per `.agents/PLANS.md`, ExecPlans are living documents even after their original milestones are complete; adding a new milestone here (rather than silently editing history) keeps the plan an accurate, sequential record of what changed and why.
  Date/Author: 2026-07-02, addendum recorded during VIX-2690 requirements clarification with Jeff Uchitjil.

## Outcomes & Retrospective

All 7 milestones are complete. Shipped: `ElementTagDefinition` and `ElementTagCollection` core data model, `Tags` on `IElementNode`/`ElementNode`/`ProxyElementNode`, the `ElementTagService` catalog service (CRUD, built-in resolution, validation, cross-node cleanup), and full XML persistence (`SystemConfig.Tags`, the two new serializers, `SystemConfigFilePolicy` wiring, per-node `<Tags>` persistence, and `VixenSystem.TagDefinitions` load/save/seed/cleanup). 45 new unit tests were added across the five implementation milestones; the full suite (110 tests) passes and the solution builds clean.

Deferred, as planned from the outset: Display Setup/Sequencer UI, filtering, sorting, and visual display of tags — these are explicit non-goals of this plan and belong in a follow-on ticket once this API exists. Also deferred per Milestone 5's own note: dedicated automated coverage of `XmlElementNodeSerializer`'s `<Tags>` read/write path was not added (no `InternalsVisibleTo`-free way to construct the serializer in isolation was pursued beyond a short investigation); this path was instead covered by Milestone 7's manual verification.

Lessons learned: splitting a property's declaration (Milestone 4, stub) from its full wiring (Milestone 5) resolved a real dependency-order tension in the plan without breaking the "solution must compile after every milestone" rule — worth calling out explicitly in future plans that have a similar service-depends-on-not-yet-persisted-data shape. Also, before deviating from a generic project skill's stricter preferences (e.g. `dotnet-best-practices`'s structured-logging and `sealed`-class guidance), grep the surrounding codebase first — in this repo, string-interpolated NLog calls and non-sealed internal classes are the actual established convention, and matching that consistently beat conforming only the new code to the generic checklist.

JIRA issue VIX-3933 was not manually transitioned; the team's workflow transitions it automatically when the PR is created.

Addendum (2026-07-02): Milestone 8 was added after this retrospective was first written, to give the built-in tags default `DisplayColor` values. That milestone is not yet implemented — see its Progress entry above for what remains. This retrospective paragraph intentionally still describes only the original 7 milestones' completed state; update it again once Milestone 8 lands.
