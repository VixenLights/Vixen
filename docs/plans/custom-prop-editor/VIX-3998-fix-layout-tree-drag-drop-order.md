...
- [x] (2026-09-12 14:28Z) Milestone 4: Full validation completed. The Release/x64 `Vixen_Tests` MSBuild target succeeded, all 932 unit tests passed, and `git diff --check` was clean. Manual Custom Prop Editor testing confirmed ordinary and Control-modified multi-selection moves upward and downward; before and after drops including after the final sibling; noncontiguous selections; drops within the selected block; and cross-group moves all retain exact order without a crash. Saving and reopening retained the hierarchy, sibling order, lights, and group descendants. Posted this evidence to [VIX-3998](https://vixenlights.atlassian.net/browse/VIX-3998); its acceptance criteria required no reconciliation.
...
- Decision: Leave VIX-3998 in its existing In Progress workflow state. The final evidence was posted, but no separate direction authorized a Jira transition.
  Rationale: The plan expressly prohibits transitioning the issue without separate direction.
  Date/Author: 2026-09-12 / Codex
...
All milestones are complete. `PropModelServices.TryMoveWithinParent` converts a validated pre-mutation insertion slot to a valid final `Move` index while preserving input order, and `ElementTreeViewModel.Drop` routes same-parent sibling moves through that operation exactly once. The public drop tests cover the reported crash case, ordering variants, collection/view-model identity, cross-parent parent and light preservation, and center precedence. The Release/x64 test target succeeded, all 932 unit tests passed, and `git diff --check` was clean. Manual validation confirmed upward, downward, before, after, final-sibling, noncontiguous, Control-modified, within-selection, and cross-group behavior; no crash occurred and multi-item order was correct. Saving and reopening preserved the hierarchy, sibling order, lights, and group descendants. Final evidence was posted to VIX-3998. No remaining validation gaps are known.
...
Revision note (2026-09-12): Completed Milestone 4. The Release/x64 build succeeded, all 932 unit tests passed, `git diff --check` was clean, and manual drag/drop plus save/reopen validation confirmed the corrected order and absence of the crash. Posted final evidence to VIX-3998 without transitioning its workflow state.
