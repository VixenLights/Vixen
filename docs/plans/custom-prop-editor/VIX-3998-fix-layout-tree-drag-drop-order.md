...
- [ ] Milestone 3: Refactor `ElementTreeViewModel.Drop` to consume insertion slots correctly and add the public-boundary drag/drop regression suite.
...
Milestone 2 is complete. `PropModelServices.TryMoveWithinParent` now converts a validated pre-mutation insertion slot to a valid final `Move` index while preserving input order; all focused service tests pass. The public `ElementTreeViewModel.Drop` method still contains the old `InsertIndex` arithmetic, so the user-visible VIX-3998 drag/drop defect is not fixed until Milestone 3 is complete. Update this section after every major milestone with what was actually delivered, any gaps, and the build, test, and manual-validation results. At completion, compare the observed tree order, identity, parent IDs, lights, save/reopen state, and invalid-request behavior against the acceptance criteria below.
