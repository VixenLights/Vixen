...
- [x] (2026-08-08) Post-Milestone-3 fixes from manual testing: (1) narrowed the `try`/`finally` so it ends right after the download loop instead of wrapping the dialog and `LoadVendorModel` call too, so the wait cursor now clears before the vendor selection dialog opens instead of staying stuck for the dialog's lifetime; (2) reordered the two `StatusBarItem`s in `CustomPropEditorWindow.xaml` so the mouse-coordinates item stays leftmost and `StatusMessageBarItem` renders to its right, matching `StatusBar`'s left-to-right layout order. Rebuilt `CustomPropEditor.csproj` successfully with 0 errors after both fixes.
- [x] (2026-08-08) Milestone 4: `dotnet build src\Vixen.Modules\App\CustomPropEditor\CustomPropEditor.csproj --no-restore` succeeded (0 errors); `msbuild Vixen.sln -m -restore -t:Vixen_Tests -p:Configuration=Release -p:Platform=x64 -p:PlatformTarget=x64 -v:m` succeeded (0 errors, exit code 0); `dotnet test src\Vixen.Tests\Vixen.Tests.csproj -c Release --no-build --no-restore -p:Platform=x64 -p:SolutionDir="$(pwd)/" --filter "FullyQualifiedName~App.CustomPropEditor"` passed 130/130 tests, 0 failed. `git diff --check` produced no output. User-confirmed manual validation covers all three scenarios: (1) successful path — cursor/status appear and clear correctly, dialog opens promptly with no lingering wait cursor; (2) network failure (tested by pointing `GetVendorUrls()` at a bad URL) — the existing `IMessageBoxService.ShowError` dialog appears with no wait cursor showing over it, and `StatusMessage` clears once the user dismisses the dialog and the method's `finally` runs; (3) repeat runs reset cleanly with no leaked cursor/message state. Jira comment posted to VIX-3968 with build/test results, manual validation summary, and commit references; issue left in its existing status, no workflow transition made.

## Surprises & Discoveries

- Observation: Milestone 4's `dotnet build src\Vixen.Modules\App\CustomPropEditor\CustomPropEditor.csproj --no-restore` step (run without an explicit `-c`/`-p:Configuration` flag, exactly as this plan specifies) writes its output into an untracked `C:\Dev\Vixen\Output\` folder rather than the gitignored `Debug\Output\`/`Release\Output\` folders, because `$(Configuration)` is apparently empty when `BaseOutputPath` (`$(SolutionDir)$(Configuration)\Output` in `src/Directory.Build.props`) is evaluated for a standalone project build. `.gitignore` only excludes `/Release`, `/Release64`, `Debug`, and `Debug64` — a bare repo-root `Output/` is not covered. Something in the editor environment (most likely Rider's VCS auto-add setting) staged the newly written files in that folder as new (`git status` showed dozens of `A  Output/...` entries) even though nothing in this session ran `git add` on that path.
  Evidence: `git status --short` after the build showed `A  Output/AudioPlayer.dll`, `A  Output/Module.App.CustomPropEditor.dll`, etc. `git check-ignore -v Output/AudioPlayer.dll` produced no match. `grep -n Output .gitignore` produced no match.
- Decision: Ran `git reset -- Output/` to unstage those files (non-destructive; only removes them from the index, not disk) rather than committing or deleting them. Left `.gitignore` and `src/Directory.Build.props` unchanged pending explicit direction, since both are outside VIX-3968's scope and the latter was touched very recently by an unrelated, deliberate commit (`f4fdd3591`).
  Date/Author: 2026-08-08 / Claude

- Observation: The user reported that `f4fdd3591` (an earlier, unrelated fix defaulting `SolutionDir` when empty) was meant to solve exactly this bare-`Output/` problem but the folder kept reappearing on every build made while working this plan. Root cause: `Directory.Build.props` is imported before the .NET SDK applies its own default for `$(Configuration)`, so a standalone `dotnet build <project>.csproj` invocation without an explicit `-c`/`-p:Configuration` sees `$(Configuration)` as empty at the point `BaseOutputPath` (`$(SolutionDir)$(Configuration)\Output`) is evaluated, producing a bare `Output` segment instead of `Debug\Output`. `f4fdd3591` only defaulted `$(SolutionDir)`, not `$(Configuration)`, so this half of the same class of bug was still live.
  Evidence: Before the fix, `dotnet build ... --no-restore` (no `-c`) reported `CustomPropEditor -> C:\Dev\Vixen\Output\Module.App.CustomPropEditor.dll`. After adding a `Configuration Condition="'$(Configuration)' == ''">Debug<` default to `src/Directory.Build.props` (mirroring the existing `SolutionDir` default), the identical command reported `CustomPropEditor -> C:\Dev\Vixen\Debug\Output\Module.App.CustomPropEditor.dll`, and `-c Release` still correctly produced `Release\Output\`.
- Decision: Fixed `src/Directory.Build.props` at the user's explicit request (outside VIX-3968's own scope, but a direct follow-up to the discovery above and to their prior `f4fdd3591` fix). Deleted the stale, now-superseded `Output/` folder from the working tree (it was fully untracked; `git ls-files -- Output/` returned nothing) rather than leaving it to cause confusion. This repo-wide build-config change should land as its own commit, separate from VIX-3968's feature commits, matching `f4fdd3591`'s own `fix:` commit style rather than a `VIX-3968` prefix.
  Date/Author: 2026-08-08 / Claude
...
- Observation: The initial Milestone 3 implementation wrapped the entire `OpenVendorBrowserAsync` body — including `await uiVisualizerService.ShowDialogAsync(vm)` and the subsequent `LoadVendorModel` call — inside the busy-indicator `try`/`finally`. Manual testing showed the wait cursor stayed active for the whole time the vendor selection dialog was open, not just during the downloads, because `finally` only runs once the method returns and the method didn't return until after the dialog closed and (if a model was picked) `LoadVendorModel` finished.
  Evidence: User-reported manual testing: "The wait cursor does not disappear when the Vendor dialog opens."
- Decision: Ended the `try`/`finally` immediately after the download loop (right after the `if (!vendorInventories.Any()) { return; }` check), so `StatusMessage`/`pleaseWaitService.Hide()` fire before `uiVisualizerService.ShowDialogAsync` is ever called. The dialog-showing and `LoadVendorModel`-calling code now runs after the `try`/`finally` block, unprotected by it — `LoadVendorModel` already manages its own independent `Show()`/`Hide()` pair, so this does not leave any gap.
  Date/Author: 2026-08-08 / Claude
- Observation: The initial Milestone 2 XAML change added `StatusMessageBarItem` as the first `StatusBarItem` in the `StatusBar`, ahead of the existing mouse-coordinates item. Since `StatusBar` lays out items left to right in document order, this pushed the coordinates item to the right instead of leaving it anchored at the left edge.
  Evidence: User-reported manual testing: "The status message should be to the right of the coordinates status item so the coordinates remain anchored to the left."
- Decision: Reordered the two `StatusBarItem` elements so the coordinates item comes first (leftmost) and `StatusMessageBarItem` comes second (to its right). No binding, trigger, or naming changed — only document order.
  Date/Author: 2026-08-08 / Claude
...
- Decision: Do not add an artificial `Task.Delay` before the first network call, unlike the CPU-bound fix in VIX-3563.
  ...
  Date/Author: 2026-08-08 / Claude
  Verified: 2026-08-08 — user-confirmed manual testing of the successful path showed the wait cursor appearing immediately with no visible delay. No `Task.Delay` was needed; this decision holds without reversal.

## Outcomes & Retrospective

VIX-3968 is complete. Choosing `Tools > Vendor Browser` in the Custom Prop Editor now shows the standard Windows wait cursor and a status-bar message ("Loading vendor list...", then "Retrieving inventory from `<Vendor>`...") for the duration of the vendor-list and inventory downloads, both clearing before the selection dialog opens, before an early return (no vendors, all downloads failed), and after an unhandled exception. The status message itself was implemented as a general-purpose `StatusMessage` property on `PropEditorViewModel`, not scoped to the vendor browser, so a future change can reuse it for import/save/export without renaming or restructuring anything.

Manual testing surfaced two real bugs beyond the original design (the wait cursor staying active through the whole vendor-selection dialog instead of clearing before it opened, and the new status item pushing the mouse-coordinate item off the left edge), both fixed and re-verified. Testing also surfaced an unrelated, pre-existing build-configuration bug — `Directory.Build.props` left `$(Configuration)` unset for standalone project builds, scattering untracked build output into a bare `Output/` folder outside `.gitignore`'s coverage — which was root-caused and fixed as its own commit at the user's request, distinct from this ticket's scope.

All acceptance criteria from the Jira description are met and manually verified: the wait cursor and status message appear immediately and update per vendor on the successful path; both clear before the dialog opens and reliably reset across repeated runs; the network-failure path still shows the existing `IMessageBoxService.ShowError` dialog (with no wait cursor obscuring it) and clears the status message once the dialog is dismissed; `LoadVendorModel`'s own independent busy-indicator handling is unaffected. The "no artificial `Task.Delay`" design decision held — the cursor appeared without any visible delay, unlike the CPU-bound case in VIX-3563 that required one.

Automated coverage: `dotnet build` on `CustomPropEditor.csproj` (0 errors), the full `Vixen_Tests` msbuild target (0 errors), and the `App.CustomPropEditor` `dotnet test` filter (130/130 passed) all succeeded, with `git diff --check` clean throughout. This area of the codebase has no dedicated automated tests for `PropEditorViewModel` itself, so correctness here rests on manual validation plus the regression coverage the existing importer/model tests under the same namespace provide.

Commits, in order: `cd6afa709` (pre-existing warning cleanup, separated per user request), `8fe66cbe6` (`StatusMessage` property + status-bar UI), `95b58227b` (wire busy cursor/status into `OpenVendorBrowserAsync`), `f444b92b9` (fix cursor scope + status item order from testing), `1a74b6935` (Milestone 4 progress), `cbb7f34fd` (unrelated `Directory.Build.props` fix, plain `fix:` commit), `8827943b1` (record that fix in this plan). Nothing remains outstanding for this ticket.
