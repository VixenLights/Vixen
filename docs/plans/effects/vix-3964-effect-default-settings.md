...
- [ ] Milestone 2: Implement core storage — `ExcludeFromEffectDefaultAttribute` and its ten applications, `EffectDefaultsStore`/`EffectDefaultEntry`, `EffectDefaultsService`, `EffectDefaultScrubber`, plus automated tests. No user-visible behavior change yet.
- [ ] Milestone 3: Editor integration — two new effect-editor header buttons and the six Timed Sequence Editor call sites that must materialize new effects from the saved defaults. First user-visible behavior.
- [ ] Milestone 4: Export, import, and the on-demand readable diagnostic dump.
- [ ] Milestone 5: Documentation under `docs/effects/`.
...
Not yet started. This section will be filled in as each milestone completes.
