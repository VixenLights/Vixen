...
- [x] (2026-08-09) Post-Milestone-3 tweak: the Save/Reset header buttons now use a new `ToolbarIconButtonStyle` (flat/transparent at rest, subtle highlighted border on hover, sunken on press, dimmed when disabled), defined once in the shared `src/Vixen.Common/WPFCommon/Theme/Theme.xaml` and referenced via `StaticResource` from both `CategorizedLayout.xaml` and `AlphabeticalLayout.xaml`, instead of the default raised/gradient `Button` style, so they read as toolbar-style icon buttons without an actual `ToolBar` control. See `Decision Log`.
- [x] (2026-08-10) Post-Milestone-4 fix from manual testing: `WriteDiagnosticDump` no longer leaves each entry's `Payload` as an opaque, unreadable base64 blob in the dump. It now deserializes each entry's `ModuleData` (when the effect type is still installed) and inlines the actual settings as readable XML elements; an entry whose effect type is no longer installed gets an explanatory XML comment instead of a raw byte dump. See `Decision Log`.
...
- Decision (partially superseded, kept for history — see the `WriteDiagnosticXml` decision below): `EffectDefaultsService.Export`, `Import`, and `WriteDiagnosticDump` each delegate their disk-independent core logic to a new `internal static` helper (`BuildExportStore`, `MergeEntries`, `WriteIndentedXml` respectively), mirroring the `CaptureScrubbedPayload` extraction from Milestone 2, and Milestone 4's automated tests call those helpers directly instead of going through `EffectDefaultsService.Instance`. `WriteIndentedXml` was later replaced by `WriteDiagnosticXml`; `BuildExportStore` and `MergeEntries` are unaffected and still current.
...
- Decision (superseded, kept for history): Added a new named `Style x:Key="ToolbarIconButtonStyle"` duplicated in both `CategorizedLayout.xaml` and `AlphabeticalLayout.xaml`, matching the existing pattern of duplicating the whole header `Grid` across both files for template parity, rather than reusing or modifying the app's global implicit `Button` style in `src/Vixen.Common/WPFCommon/Theme/Theme.xaml`.
  Rationale (as originally recorded, now superseded — see below): The user asked for the Save/Reset buttons to look like toolbar icon buttons (flat/transparent at rest, highlighted on hover, sunken on press) without adding an actual `ToolBar` control. A codebase search found no existing reusable "flat toolbar button" style — the app's only global `Button` style (`Theme.xaml:293-390`) is a raised/gradient button meant for normal dialog buttons, and the only icon-only button precedent (`CloseButton`, `Theme.xaml:2023-2097`) is a bespoke shape-based close glyph, not a generic reusable style. Modifying the global `Button` style would have changed the appearance of every other button in the application, which is out of scope and risky.
  Date/Author: User-requested tweak after manual testing, 2026-08-09; superseded same day.

- Decision: Moved `ToolbarIconButtonStyle` out of the two layout files entirely and into `src/Vixen.Common/WPFCommon/Theme/Theme.xaml` (placed immediately after the existing `CloseButton` icon-button style, `Theme.xaml:2023-2097`, as a second reusable icon-button precedent), and had `CategorizedLayout.xaml`/`AlphabeticalLayout.xaml` reference it via `StaticResource` after adding an explicit `ResourceDictionary.MergedDictionaries` entry for `pack://application:,,,/WPFCommon;component/Theme/Theme.xaml` — the exact same pack URI already used for the same purpose in `src/Vixen.Modules/Editor/EffectEditor/Themes/EditorResources.xaml:15`.
  Rationale: The user explicitly asked not to duplicate the style and to put it in the shared theme file instead. Merging `Theme.xaml` directly into each layout file's own `ResourceDictionary.MergedDictionaries` (rather than relying on `Themes/Generic.xaml:12` — `src/Vixen.Modules/Editor/EffectEditor/Themes/Generic.xaml`, which already merges `CategorizedLayout.xaml` — to make `Theme.xaml` visible as a sibling) was chosen because WPF's `Themes/Generic.xaml` convention is a private per-assembly theme-lookup dictionary, not something automatically merged into `Application.Resources`; `StaticResource` references inside `CategorizedLayout.xaml`'s own markup only reliably resolve against dictionaries `CategorizedLayout.xaml` itself merges, not against unrelated sibling entries in whatever outer dictionary happens to reference it. Merging `Theme.xaml` directly avoids depending on that ambiguity. The new style's hover/pressed/disabled colors (`#20FFFFFF` hover fill, `#686868` hover border, `#30000000` pressed fill, `#555555` pressed border) still match the hover/pressed border and fill tones already used by the global `Button` style's `IsMouseOver`/`IsPressed` triggers, so the buttons read as consistent with the rest of the dark theme.
  Date/Author: User-requested correction after manual testing, 2026-08-09.

- Decision: Replaced `WriteIndentedXml` (which serialized the whole in-memory `EffectDefaultsStore`, `Payload` byte arrays included, through a plain `DataContractSerializer`) with `WriteDiagnosticXml`, which writes each entry's metadata by hand and, for each entry whose effect type is still installed, deserializes `Payload` back into its real `ModuleData` object and inlines that object's fields as ordinary XML elements via `DataContractSerializer.WriteObjectContent`. An entry whose effect type is no longer installed gets an XML comment explaining that its settings are unavailable, instead of a raw byte dump.
  Rationale: `DataContractSerializer` has no special handling for `byte[]` — it emits it as a base64-encoded string, which is technically text but not remotely "human-readable": a user or support responder cannot tell what curve shape, gradient colors, or other settings a saved default holds by looking at a blob of base64. This defeats the entire stated purpose of the diagnostic dump ("for troubleshooting... so you... can inspect exactly what's stored," per `docs/effects/effect-defaults.md`). The original Milestone 4 rationale — serializing the exact same in-memory object graph used for the binary store "so it can never drift out of sync with what is actually stored" — is preserved in spirit: `WriteDiagnosticXml` still deserializes the exact `Payload` bytes that are actually persisted (not a separately-tracked copy), so the dump can never show settings that don't match what is really stored; it just also resolves those bytes into their real, readable form instead of stopping at the byte layer. `DataContractSerializer.WriteObjectContent` (rather than `WriteObject`) is used so each entry's deserialized `ModuleData` fields nest directly inside a hand-written `<ModuleData>` element alongside that entry's own metadata, instead of each entry emitting its own independent root element that would be awkward to nest.
  Date/Author: User-requested fix after manual testing, 2026-08-10.
...
Implement `EffectDefaultsService.WriteDiagnosticDump(string path)` (updated 2026-08-10 after manual testing found the original approach unreadable — see `Decision Log`): open an `XmlWriter.Create(path, new XmlWriterSettings { Indent = true })` and, for each entry in the in-memory store, write its `TypeId`/`EffectName`/`DataModelTypeName`/`SavedUtc` metadata as plain elements, then resolve the entry's `ModuleData` type via `Modules.GetDescriptorById(entry.TypeId)?.ModuleDataClass` and, if resolved, deserialize `entry.Payload` with a `DataContractSerializer` for that type and inline the deserialized object's fields directly into the XML via `DataContractSerializer.WriteObjectContent` — never leaving `Payload` as a raw, unreadable byte/base64 blob in the output. If the type cannot be resolved (the effect's module is no longer installed), write an XML comment explaining that instead. The dump still reads from `Payload` exactly as persisted, so it cannot drift from what is actually stored — it just also resolves those bytes into their real, human-readable settings.

Add tests covering: export writes only the requested subset; import merges new entries and overwrites existing entries for the same `TypeId` without disturbing untouched entries; the diagnostic dump is valid, indented XML containing the same `TypeId`/`EffectName`/`SavedUtc` values as the in-memory store, and (added 2026-08-10) the dump inlines an entry's actual deserialized settings as readable elements rather than a base64 payload blob when its effect type is installed, and explains unavailability in a comment when it is not.
