...
- [x] Milestone 5 completed (2026-09-14): validated full-track mark collection generation and the existing-defect fixes with real audio, including the responsive progress dialog, diagnostic output, and corrected green Beat Splits collections. The full automated suite passed 936 tests with zero failures. Subsequent standards-only cleanup was completed separately with no logic changes.
...
- Decision: Log processed PCM-block and feature-output counts, warn only for missing expected output indexes, and do not retry or fabricate a sparse analysis result.
  Rationale: a retry could silently change user-visible timing marks and fabricating data would conceal an upstream tracker failure. The logs provide the concrete evidence needed to distinguish a missing native output from an output that is present but empty, while preserving existing behavior.
  Date/Author: 2026-09-14 / Codex.
...
All five milestones are complete. `QmBarBeatTrackTests` directly exercises the C++/CLI wrapper with generated mono PCM, verifies the VAMP 2.10 descriptor set and input domain, retains the click-track feature baseline, and proves that disposing the managed wrapper releases its native plugin and prevents subsequent use. The deterministic 44.1 kHz click-track timestamps and feature counts remained unchanged. QM 1.8.0 adds optional `alpha`, `inputtempo`, and `constraintempo` descriptors, but `BeatsAndBars.cs` continues to require only `bpb`. Full-track generation now keeps the modal progress dialog responsive while native processing runs on a worker thread, and changes the dialog to a marquee finalization state before `GetRemainingFeatures()` performs the global tempo calculation. Preview and full-track native trackers are now deterministically disposed. Full-track logging records processed-block and per-output feature counts, and warns when QM omits an expected output index without changing the result. Real-audio verification confirmed the expected mark collection generation, the progress-dialog correction, diagnostic output, and the corrected green Beat Splits collections. A second real-audio comparison had no differences apart from regenerated collection IDs; observed timing differences in the affected comparison were reviewed as detector-output changes rather than unexplained regressions. The full automated suite passed 936 tests with zero failures. A separate follow-up commit performed standards-only cleanup without changing logic. No further work remains in this plan.
