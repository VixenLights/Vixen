...
namespace Dataweb.NShape.Advanced
{
	...
	public class PictureBase : RectangleBase
	{
		...
		public override void Draw(Graphics graphics)
		{
			...
			if (image != null && image.Image is Metafile) {
				GdiHelpers.DrawImage(graphics, image.Image, imageAttribs, imageLayout, imageBounds, imageBounds,
				                     Geometry.TenthsOfDegreeToDegrees(Angle), Center);
			}
			else {
		...
		protected override void RecalcDrawCache()
		{
			...
			imageDrawBounds[0].X = imageBounds.Left;
			imageDrawBounds[0].Y = imageBounds.Top;
			imageDrawBounds[1].X = imageBounds.Right;
			imageDrawBounds[1].Y = imageBounds.Top;
			imageDrawBounds[2].X = imageBounds.Right;
			imageDrawBounds[2].Y = imageBounds.Bottom;
			imageDrawBounds[3].X = imageBounds.Left;
			imageDrawBounds[3].Y = imageBounds.Bottom;

			if (imageAttribs == null)
				imageAttribs = GdiHelpers.GetImageAttributes(imageLayout, imageGamma, imageTransparency, imageGrayScale, isPreview,
				                                             transparentColor);
			...
			if (bitmapImg is Bitmap && imageBrush == null) {
				...
				if (Angle != 0)
					imageCenter = Geometry.RotatePoint(Point.Empty, Geometry.TenthsOfDegreeToDegrees(Angle), imageCenter);
				GdiHelpers.TransformTextureBrush(imageBrush, imageLayout, imageBounds, imageCenter,
				                                 Geometry.TenthsOfDegreeToDegrees(Angle));
			}
		}
	...
	public class ImageBasedShape : ShapeBase, IPlanarShape, ICaptionedShape
	{
		...
		public ICharacterStyle CharacterStyle
		{
			...
			set
			{
				charStyle = (Template != null && value == ((ICaptionedShape) Template.Shape).GetCaptionCharacterStyle(0))
				            	? null
				            	: value;
		...
		public IParagraphStyle ParagraphStyle
		{
			...
			set
			{
				paragraphStyle = (Template != null && value == ((ICaptionedShape) Template.Shape).GetCaptionParagraphStyle(0))
				                 	? null
				                 	: value;
				caption.InvalidatePath();
				Invalidate();
		...
		public override Point CalculateNormalVector(int x, int y)
		{
			if (!ContainsPoint(x, y))
				throw new NShapeException("Coordinates {0} are outside {1}.", new Point(x, y), Type.FullName);
		...
		[Category("Appearance")]
		[Description(
			"Defines the appearence of the shape's interior.\nUse the template editor to modify all shapes of a template.\nUse the design editor to modify and create styles."
			)]
		...
		/// <ToBeCompleted></ToBeCompleted>
		public bool GetCaptionTextBounds(int index, out Point topLeft, out Point topRight, out Point bottomRight,
		                                 out Point bottomLeft)
		{
			...
			Geometry.TransformRectangle(location, Angle, captionBounds, out topLeft, out topRight, out bottomRight,
			                            out bottomLeft);
			return true;
		}
		...
		/// <ToBeCompleted></ToBeCompleted>
		public bool GetCaptionBounds(int index, out Point topLeft, out Point topRight, out Point bottomRight,
		                             out Point bottomLeft)
		{
			if (index != 0) throw new IndexOutOfRangeException("index");
			Point location = Point.Empty;
		...
		{
			return Geometry.RectangleContainsPoint(this.x - this.w/2, this.y - this.h/2 + this.h - this.ch, this.w, this.ch, x, y)
			       	? 0
			       	: -1;
		}
		...
		protected override ControlPointId GetControlPointId(int index)
		{
			switch (index) {
				...
				case 3:
					return 8;
				default:
					throw new NShapeException("NotSupported control point index.");
			}
		}
		...
		{
			bool result = Geometry.RectangleIntersectsWithRectangle(this.x - this.w/2, this.y - this.h/2, this.w, this.h, x, y,
			                                                        width, height);
			return result;
		}
		...
		private void Construct(string resourceBaseName, Assembly resourceAssembly)
		{
			...
			if (stream == null)
				throw new ArgumentException(
					string.Format("'{0}' is not a valid resource in '{1}'.", resourceBaseName, resourceAssembly), "resourceBaseName");
			image = Image.FromStream(stream);
			if (image == null)
				throw new ArgumentException(string.Format("'{0}' is not a valid image resource.", resourceBaseName),
				                            "resourceBaseName");
	...
	public class CustomizableMetaFile : ImageBasedShape
	{
		...
		private void FindRemapColor()
		{
			using (Graphics gfx = Graphics.FromHwnd(IntPtr.Zero)) {
				MetafileHeader header = ((Metafile) image).GetMetafileHeader();
				gfx.EnumerateMetafile((Metafile) image, header.Bounds, header.Bounds, GraphicsUnit.Pixel, findReplaceColorCallback,
				                      IntPtr.Zero);
			}
		}
