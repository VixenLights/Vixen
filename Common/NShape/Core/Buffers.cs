...
namespace Dataweb.Utilities
{
	...
	internal class BufferStringIterator : IStringIterator {
		...
		public enum Format { AnsiBytePrefixed, AnsiWordPrefixed, WideWordPrefixed };
	...
	internal class Blob : IComparable {
		...
		const int DefBlobCapacityKB = 100;
		...
		public int CompareTo(object obj) {
			int result = 0;
			if (!(obj is Blob)) throw new ArgumentException("Blob cannot compare to another blob: compare-value is not of type Blob", "obj");
		...
		internal void SetCapacity(int capacity) {
			if (capacity <= 0 || capacity > MaxCapacity)
				throw new ArgumentOutOfRangeException("Capacity", "Capacity of a blob must satisfy condition: c e [1, " + MaxCapacity.ToString() + "]");
			if (capacity < length)
				throw new ArgumentOutOfRangeException("Capacity", "Capacity of a blob must be greater or equal as the current length");
			if (data != null) {
		...
		internal void Append(byte[] buffer, int bufferOffset, int size) {
			if (bufferOffset + size > buffer.Length)
				throw new ArgumentException("Cannot append data from buffer to blob: buffersize is not sufficient", "bufferOffset + size");
		...
		internal void CopyTo(byte[] buffer, int bufferOffset, int index, int size) {
			if (bufferOffset + size > buffer.Length)
				throw new ArgumentException("Cannot copy data from blob to buffer: buffersize is not sufficient", "bufferOffset + size");
			if (index + size > Length)
				throw new ArgumentException("Cannot copy data from blob to buffer: blob does not provide sufficient data", "start + size");
	...
	internal class DateExtensions {
		...
		internal static int WeekOfYear(DateTime dt) {
			...
			else if (firstWeeksMonday.DayOfWeek >= DayOfWeek.Thursday) firstWeeksMonday = firstWeeksMonday.AddDays(firstWeeksMonday.DayOfWeek - DayOfWeek.Thursday + 4);
