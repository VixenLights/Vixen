...
namespace Dataweb.NShape.Commands
{
	...
	public abstract class Command : ICommand
	{
		...
		protected abstract bool CheckAllowedCore(ISecurityManager securityManager, bool createException,
	...
	public abstract class ShapeCommand : Command
	{
		...
		/// <override></override>
		protected override bool CheckAllowedCore(ISecurityManager securityManager, bool createException,
		                                         out Exception exception)
		{
	...
	public abstract class AutoDisconnectShapesCommand : Command
	{
		...
		/// <ToBeCompleted></ToBeCompleted>
		protected Dictionary<Shape, List<ShapeConnectionInfo>> ShapeConnections
		{
	...
	public abstract class InsertOrRemoveModelObjectsCommand : Command
	{
		...
		protected void RemoveModelObjects(bool deleteShapes)
		{
			if (ModelObjects.Count == 0)
				throw new NShapeInternalException("No ModelObjects set. Call SetModelObjects() before.");
			if (Repository != null) {
				foreach (KeyValuePair<IModelObject, AttachedObjects> item in ModelObjects)
	...
	public abstract class InsertOrRemoveShapeCommand : AutoDisconnectShapesCommand
	{
		...
		{
			switch (descType) {
				case DescriptionType.Insert:
					return "Create";
				case DescriptionType.Delete:
					return "Delete";
				case DescriptionType.Cut:
					return "Cut";
				case DescriptionType.Paste:
					return "Paste";
				default:
		...
		private IEnumerable<TEntity> GetEntities<TEntity>(IEnumerable<TEntity> entities, Predicate<TEntity> predicate)
	...
	{
		/// <ToBeCompleted></ToBeCompleted>
		protected ConnectionCommand(Shape connectorShape, ControlPointId gluePointId, Shape targetShape,
		...
		protected ConnectionCommand(IRepository repository, Shape connectorShape, ControlPointId gluePointId,
		                            Shape targetShape, ControlPointId targetPointId)
			: base(repository)
		{
	...
	public abstract class ShapeAggregationCommand : AutoDisconnectShapesCommand
	{
		...
		/// <ToBeCompleted></ToBeCompleted>
		protected ShapeAggregationCommand(IRepository repository, Diagram diagram, Shape aggregationShape,
		...
		private void CreateLabelString()
		{
			...
			if (commands.Count > 0) {
				...
				for (int i = 1; i <= lastIdx; ++i) {
					...
					description += string.Format("{0}{1}{2}", newLine, commands[i].Description.Substring(0, 1).ToLowerInvariant(),
					                             commands[i].Description.Substring(1));
				}
			}
		}
	...
	public class InsertShapeCommand : CreateShapesCommand
	{
		...
		/// <ToBeCompleted></ToBeCompleted>
		public InsertShapeCommand(IRepository repository, Diagram diagram, LayerIds layerIds, Shape shape,
		                          bool withModelObjects, bool keepZOrder)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public InsertShapeCommand(IRepository repository, Diagram diagram, LayerIds layerIds, Shape shape,
		                          bool withModelObjects, bool keepZOrder, int toX, int toY)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public InsertShapeCommand(Diagram diagram, LayerIds layerIds, IEnumerable<Shape> shapes, bool withModelObjects,
		                          bool keepZOrder, int deltaX, int deltaY)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public InsertShapeCommand(IRepository repository, Diagram diagram, LayerIds layerIds, IEnumerable<Shape> shapes,
		                          bool withModelObjects, bool keepZOrder, int deltaX, int deltaY)
	...
	public class CreateShapesCommand : InsertOrRemoveShapeCommand
	{
		...
		/// <ToBeCompleted></ToBeCompleted>
		public CreateShapesCommand(IRepository repository, Diagram diagram, LayerIds layerIds, Shape shape,
		                           bool withModelObjects, bool keepZOrder)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public CreateShapesCommand(Diagram diagram, LayerIds layerIds, IEnumerable<Shape> shapes, bool withModelObjects,
		                           bool keepZOrder, int deltaX, int deltaY)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public CreateShapesCommand(IRepository repository, Diagram diagram, LayerIds layerIds, IEnumerable<Shape> shapes,
		                           bool withModelObjects, bool keepZOrder, int deltaX, int deltaY)
			: base(repository, diagram)
		{
			if (diagram == null) throw new ArgumentNullException("diagram");
	...
	public class MoveShapeByCommand : ShapesCommand
	{
		...
		public override void Execute()
		{
			...
			if (connectionsBuffer != null) {
				for (int i = 0; i < connectionsBuffer.Count; ++i)
					connectionsBuffer[i].shape.Connect(connectionsBuffer[i].connectionInfo.OwnPointId,
					                                   connectionsBuffer[i].connectionInfo.OtherShape,
					                                   connectionsBuffer[i].connectionInfo.OtherPointId);
			}
			// update cache
		...
		private List<ConnectionInfoBuffer> connectionsBuffer;
	...
	public class MoveControlPointCommand : ShapesCommand
	{
		...
		/// <ToBeCompleted></ToBeCompleted>
		public MoveControlPointCommand(IRepository repository, Shape shape, int controlPointId, int dX, int dY,
		...
		/// <ToBeCompleted></ToBeCompleted>
		public MoveControlPointCommand(IEnumerable<Shape> shapes, int controlPointId, int dX, int dY,
		                               ResizeModifiers modifiers)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public MoveControlPointCommand(IRepository repository, IEnumerable<Shape> shapes, int controlPointId, int dX, int dY,
		                               ResizeModifiers modifiers)
	...
	public class MoveGluePointCommand : ShapeCommand
	{
		...
		/// </summary>
		public MoveGluePointCommand(Shape shape, ControlPointId gluePointId, Shape targetShape, int dX, int dY,
		                            ResizeModifiers modifiers)
		...
		/// Constructs a glue point moving command.
		/// </summary>
		public MoveGluePointCommand(IRepository repository, Shape shape, ControlPointId gluePointId, Shape targetShape, int dX,
		                            int dY, ResizeModifiers modifiers)
			: base(repository, shape)
		{
			// Find target point
			ControlPointId targetPointId = ControlPointId.None;
			if (targetShape != null) {
				targetPointId = targetShape.FindNearestControlPoint(gluePointPosition.X + dX, gluePointPosition.Y + dY, 0,
				                                                    ControlPointCapabilities.Connect);
				if (targetPointId == ControlPointId.None &&
				    targetShape.ContainsPoint(gluePointPosition.X + dX, gluePointPosition.Y + dY))
		...
		/// Constructs a glue point moving command.
		/// </summary>
		public MoveGluePointCommand(Shape shape, ControlPointId gluePointId, Shape targetShape, ControlPointId targetPointId,
		                            int dX, int dY, ResizeModifiers modifiers)
		...
		/// Constructs a glue point moving command.
		/// </summary>
		public MoveGluePointCommand(IRepository repository, Shape shape, ControlPointId gluePointId, Shape targetShape,
		                            ControlPointId targetPointId, int dX, int dY, ResizeModifiers modifiers)
			: base(repository, shape)
		{
			BaseConstruct(gluePointId, targetShape, targetPointId, dX, dY, modifiers);
		}
		...
		public override void Execute()
		{
			...
			if (!existingConnection.IsEmpty) {
				...
				if (Repository != null)
					Repository.DeleteConnection(shape, gluePointId, existingConnection.OtherShape, existingConnection.OtherPointId);
			}
			// Move point
			...
			if (!newConnection.IsEmpty) {
				...
				if (Repository != null)
					Repository.InsertConnection(shape, gluePointId, newConnection.OtherShape, newConnection.OtherPointId);
			}
		...
		public override void Revert()
		{
			...
			if (!newConnection.IsEmpty) {
				shape.Disconnect(gluePointId);
				if (Repository != null)
					Repository.DeleteConnection(shape, gluePointId, newConnection.OtherShape, newConnection.OtherPointId);
			}
			// Move point
			...
			if (!existingConnection.IsEmpty) {
				...
				if (Repository != null)
					Repository.InsertConnection(shape, gluePointId, existingConnection.OtherShape, existingConnection.OtherPointId);
			}
			if (Repository != null) Repository.Update(shape);
		}
		...
		/// <ToBeCompleted></ToBeCompleted>
		protected void BaseConstruct(int gluePointId, Shape targetShape, ControlPointId targetPointId, int dX, int dY,
		                             ResizeModifiers modifiers)
		{
			this.gluePointId = gluePointId;
			this.targetShape = targetShape;
		...
		private ControlPointId targetPointId;
		private int dX;
		private int dY;
	...
	public class RotateShapesCommand : Command
	{
		...
		/// <ToBeCompleted></ToBeCompleted>
		public RotateShapesCommand(IRepository repository, IEnumerable<Shape> shapes, int tenthsOfDegree, int rotateCenterX,
		                           int rotateCenterY)
			: base(repository)
		{
			if (shapes == null) throw new ArgumentNullException("shapes");
			...
			if (this.shapes.Count == 1)
				this.description = string.Format("Rotate {0} by {1}° at ({2}|{3}", this.shapes[0].Type.Name, tenthsOfDegree/10f,
				                                 rotateCenterX, rotateCenterY);
			else
				this.description = string.Format("Rotate {0} shapes by {1}° at ({2}|{3}", this.shapes.Count, tenthsOfDegree/10f,
				                                 rotateCenterX, rotateCenterY);
		}
	...
	public class SetCaptionTextCommand : Command
	{
		...
		/// <ToBeCompleted></ToBeCompleted>
		public SetCaptionTextCommand(IRepository repository, ICaptionedShape shape, int captionIndex, string oldValue,
		                             string newValue)
			: base(repository)
		{
			if (shape == null) throw new ArgumentNullException("shape");
			if (!(shape is Shape))
				throw new NShapeException("{0} is not of type {1}.", shape.GetType().Name, typeof (Shape).Name);
			...
			this.description = string.Format("Change text of {0} from '{1}' to '{2}", ((Shape) shape).Type.Name, this.oldValue,
			                                 this.newValue);
		}
	...
	public class LiftShapeCommand : Command
	{
		...
		private void Construct(Diagram diagram, IEnumerable<Shape> shapes, ZOrderDestination liftMode)
		{
			...
			switch (liftMode) {
				case ZOrderDestination.ToTop:
					formatStr = "Bring {0} shape{1} on top";
					break;
				case ZOrderDestination.ToBottom:
					formatStr = "Send {0} shape{1} to bottom";
					break;
				default:
					throw new NShapeUnsupportedValueException(liftMode);
			}
	...
	public class ConnectCommand : ConnectionCommand
	{
		...
		/// <ToBeCompleted></ToBeCompleted>
		public ConnectCommand(IRepository repository, Shape connectorShape, int gluePointId, Shape targetShape,
		                      int targetPointId)
	...
	public class DisconnectCommand : ConnectionCommand
	{
			...
			: base(repository, connectorShape, gluePointId)
		{
			...
			if (this.connectionInfo.IsEmpty)
				throw new NShapeInternalException(string.Format("There is no connection for Point {0} of shape {1}.", gluePointId,
				                                                connectorShape));
		}
	...
	public class AddVertexCommand : ShapeCommand
	{
		...
		{
			if (!(shape is ILinearShape))
				throw new ArgumentException(string.Format("Shape does not implement required interface {0}.",
				                                          typeof (ILinearShape).Name));
	...
	public class InsertVertexCommand : ShapeCommand
	{
			...
			: base(repository, shape)
		{
			if (!(shape is ILinearShape))
				throw new ArgumentException(string.Format("Shape does not implement required interface {0}.",
				                                          typeof (ILinearShape).Name));
			if (beforePointId == ControlPointId.None || beforePointId == ControlPointId.Any ||
			    beforePointId == ControlPointId.Reference || beforePointId == ControlPointId.FirstVertex)
	...
	public class RemoveVertexCommand : ShapeCommand
	{
			...
			: base(repository, shape)
		{
			...
			foreach (
				ControlPointId id in
					shape.GetControlPointIds(ControlPointCapabilities.Resize | ControlPointCapabilities.Connect |
		...
		public override void Execute()
		{
			if (removedPointId == ControlPointId.None)
				throw new NShapeException("ControlPointId.None is not a valid vertex to remove.");
		...
		{
			if (removedPointId == ControlPointId.None)
				throw new NShapeException("ControlPointId.None is not a valid vertex to add.");
	...
	public class AssignModelObjectCommand : Command
	{
			...
			: base(repository)
		{
			...
			if (shape.ModelObject == null)
				this.description = string.Format("Assign {0} '{1}' to {2}.", modelObject.Type.Name, modelObject.Name,
				                                 shape.Type.Name);
			else
				this.description = string.Format("Replace {0} '{1}' of {2} with {3} '{4}'.", shape.ModelObject.Type.Name,
				                                 shape.ModelObject.Name, shape.Type.Name, modelObject.Type.Name, modelObject.Name);
	...
	public class ExchangeTemplateShapeCommand : Command
	{
			...
			: base(repository)
		{
			if (originalTemplate == null) throw new ArgumentNullException("originalTemplate");
			if (changedTemplate == null) throw new ArgumentNullException("changedTemplate");
			this.description = string.Format("Change shape of tempate  '{0}' from '{1}' to '{2}'", originalTemplate.Title,
			                                 originalTemplate.Shape.Type.Name, changedTemplate.Shape.Type.Name);
			this.originalTemplate = originalTemplate;
		...
		private void ReplaceShapes(Diagram diagram, Shape oldShape, Shape newShape,
		                           IEnumerable<ShapeConnectionInfo> oldConnections,
		                           IEnumerable<ShapeConnectionInfo> newConnections)
		{
			oldShape.Invalidate();
	...
	public abstract class PropertySetCommand<T> : Command
	{
		...
		/// Initializes a new instance of <see cref="T:Dataweb.NShape.PropertySetCommand`1" />.
		/// </summary>
		public PropertySetCommand(IRepository repository, T modifiedObject, PropertyInfo propertyInfo, object oldValue,
		                          object newValue)
			: this(
				null, SingleInstanceEnumerator<T>.Create(modifiedObject), propertyInfo,
				SingleInstanceEnumerator<object>.Create(oldValue), SingleInstanceEnumerator<object>.Create(newValue))
		{
		}
		...
		/// Initializes a new instance of <see cref="T:Dataweb.NShape.PropertySetCommand`1" />.
		/// </summary>
		public PropertySetCommand(IEnumerable<T> modifiedObjects, PropertyInfo propertyInfo, IEnumerable<object> oldValues,
		                          object newValue)
		...
		/// Initializes a new instance of <see cref="T:Dataweb.NShape.PropertySetCommand`1" />.
		/// </summary>
		public PropertySetCommand(IRepository repository, IEnumerable<T> modifiedObjects, PropertyInfo propertyInfo,
		                          IEnumerable<object> oldValues, object newValue)
		...
		/// </summary>
		public PropertySetCommand(IRepository repository, IEnumerable<T> modifiedObjects, PropertyInfo propertyInfo,
		                          IEnumerable<object> oldValues, IEnumerable<object> newValues)
			: base()
		{
		...
		private void Construct(PropertyInfo propertyInfo)
		{
			...
			RequiredPermissionAttribute attr =
				Attribute.GetCustomAttribute(propertyInfo, typeof (RequiredPermissionAttribute)) as RequiredPermissionAttribute;
	...
	{
		/// <ToBeCompleted></ToBeCompleted>
		public ShapePropertySetCommand(IEnumerable<Shape> modifiedShapes, PropertyInfo propertyInfo,
		                               IEnumerable<object> oldValues, IEnumerable<object> newValues)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public ShapePropertySetCommand(IRepository repository, IEnumerable<Shape> modifiedShapes, PropertyInfo propertyInfo,
		                               IEnumerable<object> oldValues, IEnumerable<object> newValues)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public ShapePropertySetCommand(IRepository repository, Shape modifiedShape, PropertyInfo propertyInfo, object oldValue,
		                               object newValue)
	...
	{
		/// <ToBeCompleted></ToBeCompleted>
		public DiagramPropertySetCommand(IEnumerable<Diagram> modifiedDiagrams, PropertyInfo propertyInfo,
		                                 IEnumerable<object> oldValues, IEnumerable<object> newValues)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public DiagramPropertySetCommand(IRepository repository, IEnumerable<Diagram> modifiedDiagrams,
		                                 PropertyInfo propertyInfo, IEnumerable<object> oldValues,
		                                 IEnumerable<object> newValues)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public DiagramPropertySetCommand(IRepository repository, IEnumerable<Diagram> modifiedDiagrams,
		                                 PropertyInfo propertyInfo, IEnumerable<object> oldValues, object newValue)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public DiagramPropertySetCommand(IRepository repository, Diagram modifiedDiagram, PropertyInfo propertyInfo,
	...
	{
		/// <ToBeCompleted></ToBeCompleted>
		public DesignPropertySetCommand(IEnumerable<Design> modifiedDesigns, PropertyInfo propertyInfo,
		                                IEnumerable<object> oldValues, IEnumerable<object> newValues)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public DesignPropertySetCommand(IRepository repository, IEnumerable<Design> modifiedDesigns, PropertyInfo propertyInfo,
		                                IEnumerable<object> oldValues, IEnumerable<object> newValues)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public DesignPropertySetCommand(IRepository repository, Design modifiedDesign, PropertyInfo propertyInfo,
		                                object oldValue, object newValue)
	...
	{
		/// <ToBeCompleted></ToBeCompleted>
		public LayerPropertySetCommand(Diagram diagram, IEnumerable<Layer> modifiedLayers, PropertyInfo propertyInfo,
		                               IEnumerable<object> oldValues, IEnumerable<object> newValues)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public LayerPropertySetCommand(IRepository repository, Diagram diagram, IEnumerable<Layer> modifiedLayers,
		                               PropertyInfo propertyInfo, IEnumerable<object> oldValues, IEnumerable<object> newValues)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public LayerPropertySetCommand(Diagram diagram, Layer modifiedLayer, PropertyInfo propertyInfo, object oldValue,
		                               object newValue)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public LayerPropertySetCommand(IRepository repository, Diagram diagram, Layer modifiedLayer, PropertyInfo propertyInfo,
		                               object oldValue, object newValue)
	...
	{
		/// <ToBeCompleted></ToBeCompleted>
		public StylePropertySetCommand(Design design, IEnumerable<Style> modifiedStyles, PropertyInfo propertyInfo,
		                               IEnumerable<object> oldValues, IEnumerable<object> newValues)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public StylePropertySetCommand(IRepository repository, Design design, IEnumerable<Style> modifiedStyles,
		                               PropertyInfo propertyInfo, IEnumerable<object> oldValues, IEnumerable<object> newValues)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public StylePropertySetCommand(Design design, Style modifiedStyle, PropertyInfo propertyInfo, object oldValue,
		                               object newValue)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public StylePropertySetCommand(IRepository repository, Design design, Style modifiedStyle, PropertyInfo propertyInfo,
		                               object oldValue, object newValue)
	...
	{
		/// <ToBeCompleted></ToBeCompleted>
		public ModelObjectPropertySetCommand(IEnumerable<IModelObject> modifiedModelObjects, PropertyInfo propertyInfo,
		                                     IEnumerable<object> oldValues, IEnumerable<object> newValues)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public ModelObjectPropertySetCommand(IRepository repository, IEnumerable<IModelObject> modifiedModelObjects,
		                                     PropertyInfo propertyInfo, IEnumerable<object> oldValues,
		                                     IEnumerable<object> newValues)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public ModelObjectPropertySetCommand(IModelObject modifiedModelObject, PropertyInfo propertyInfo, object oldValue,
		                                     object newValue)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public ModelObjectPropertySetCommand(IRepository repository, IModelObject modifiedModelObject,
		                                     PropertyInfo propertyInfo, object oldValue, object newValue)
	...
	public class EditLayerCommand : DiagramCommand
	{
		...
		/// <ToBeCompleted></ToBeCompleted>
		public EditLayerCommand(IRepository repository, Diagram diagram, Layer layer, ChangedProperty property,
		...
		/// <ToBeCompleted></ToBeCompleted>
		public EditLayerCommand(IRepository repository, Diagram diagram, Layer layer, ChangedProperty property, int oldValue,
		                        int newValue)
			: base(repository, diagram)
		{
		...
		private void Construct(Layer layer, ChangedProperty property)
		{
			...
			description = string.Format("Change layer property '{0}' from {1}{2}{1} to {1}{3}{1}", property, quotes, oldVal,
			                            newVal);
		}
	...
	public class AssignShapesToLayersCommand : ShapesCommand
	{
		...
		/// <ToBeCompleted></ToBeCompleted>
		public AssignShapesToLayersCommand(IRepository repository, Diagram diagram, IEnumerable<Shape> shapes,
		                                   LayerIds layerIds)
			: base(repository, shapes)
		{
			if (diagram == null) throw new ArgumentNullException("diagram");
	...
	public class GroupShapesCommand : ShapeAggregationCommand
	{
		...
		/// <ToBeCompleted></ToBeCompleted>
		public GroupShapesCommand(IRepository repository, Diagram diagram, LayerIds layerIds, Shape shapeGroup,
		                          IEnumerable<Shape> childShapes)
			: base(repository, diagram, shapeGroup, childShapes)
		{
			...
			this.description = string.Format("Aggregate {0} shapes to a '{1}'", base.shapes.Count,
			                                 ((Shape) base.aggregationShape).Type.Name);
	...
	public class UngroupShapesCommand : ShapeAggregationCommand
	{
			...
			: base(repository, diagram, shapeGroup, (shapeGroup != null) ? shapeGroup.Children : null)
		{
			...
			this.description = string.Format("Release {0} shapes from {1}'s aggregation", base.shapes.Count,
			                                 base.aggregationShape.Type.Name);
			aggregationShapeOwnedByDiagram = false;
		}
	...
	{
		/// <ToBeCompleted></ToBeCompleted>
		public AggregateCompositeShapeCommand(Diagram diagram, LayerIds layerIds, Shape parentShape,
		                                      IEnumerable<Shape> childShapes)
		...
		/// <ToBeCompleted></ToBeCompleted>
		public AggregateCompositeShapeCommand(IRepository repository, Diagram diagram, LayerIds layerIds, Shape parentShape,
		                                      IEnumerable<Shape> childShapes)
