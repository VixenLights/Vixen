			...
			if (e != null && e.Value is NamedImage) {
				...
				if (img.Image != null) {
					...
					float lowestRatio =
						(float)
						Math.Round(
							Math.Min((double) (e.Bounds.Width - (e.Bounds.X + e.Bounds.X))/(double) img.Width,
							         (double) (e.Bounds.Height - (e.Bounds.Y + e.Bounds.Y))/(double) img.Height), 6);
	...
	public class StyleUITypeEditor : UITypeEditor
	{
		...
		private void DrawStyleItem(Graphics gfx, Rectangle previewBounds, ICapStyle capStyle)
		{
			...
			if (capStyle.CapSize + 2 >= previewBounds.Height) {
				scale = Geometry.CalcScaleFactor(capStyle.CapSize + 2, capStyle.CapSize + 2, previewBounds.Width - 2,
		...
		private void DrawStyleItem(Graphics gfx, Rectangle previewBounds, IFillStyle fillStyle)
		{
			...
			if (fillBrush is LinearGradientBrush) {
				...
				if (fillStyle.ImageLayout == ImageLayoutMode.Stretch) {
					...
					float scale = Geometry.CalcScaleFactor(((TextureBrush) fillBrush).Image.Width,
					                                       ((TextureBrush) fillBrush).Image.Height, previewBounds.Width,
					                                       previewBounds.Height);
					((TextureBrush) fillBrush).ScaleTransform(scale, scale);
					((TextureBrush) fillBrush).TranslateTransform((((TextureBrush) fillBrush).Image.Width*scale)/2,
					                                              (((TextureBrush) fillBrush).Image.Height*scale)/2);
				}
			}
			// Draw
		...
		private void DrawStyleItem(Graphics gfx, Rectangle previewBounds, ILineStyle lineStyle)
		{
			...
			gfx.DrawLine(linePen, previewBounds.X, previewBounds.Y + (previewBounds.Height/2), previewBounds.Right,
