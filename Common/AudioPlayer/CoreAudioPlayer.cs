...
{
    public class CoreAudioPlayer : IPlayer, IDisposable
    {
        // Singleton
        private static CoreAudioPlayer instance;
        ...
        public static CoreAudioPlayer Instance
        {
            get
            {
                if (instance == null)
                {
                    instance = new CoreAudioPlayer();
                }
                return instance;
            }
        }
        ...
        public void Play(string filename, AudioDevice audioDevice, TimeSpan startTime)
        {
            this.SetSelectedAudioDevice(audioDevice);
            ...
            try
            {
	            this.soundOut.WaveSource.SetPosition(startTime);
            }
            catch (Exception)
            {
	            // Swallow
        ...
        public class WrapperSpectrumPlayer : ISpectrumPlayer
        {
            ...
            public CoreAudioPlayer AudioPlayer;
            private readonly FftProvider fftProvider;
            private readonly ISoundOut soundOut;

            public bool IsPlaying => this.AudioPlayer.isPlaying;

            public WrapperSpectrumPlayer(CoreAudioPlayer audioPlayer, SpectrumChannel channel,
                ICollection<EventHandler<SingleBlockReadEventArgs>> inputStreamList)
            {
                this.AudioPlayer = audioPlayer;
                this.AudioPlayer.PropertyChanged += (_, __) => PropertyChanged(_, __);
                this.soundOut = audioPlayer.soundOut;
                ...
                if (channel != SpectrumChannel.Stereo)
                {
                    ...
                    {
                        if (this.AudioPlayer.notificationSource != null) this.AudioPlayer.notificationSource.SingleBlockRead += InputStream_LeftSample;
                        inputStreamList.Add(InputStream_LeftSample);
                    }
                    if (channel == SpectrumChannel.Right)
                    {
                        if (this.AudioPlayer.notificationSource != null) this.AudioPlayer.notificationSource.SingleBlockRead += InputStream_RightSample;
                        inputStreamList.Add(InputStream_RightSample);
                    }
                }
                else
                {
                    if (this.AudioPlayer.notificationSource != null) this.AudioPlayer.notificationSource.SingleBlockRead += InputStream_Sample;
