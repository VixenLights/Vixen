...
namespace Vixen.Sys
{
	internal class Modules
	{
		...
		static Modules()
		{
			...
			Directory = Path.Combine(Paths.BinaryRootPath);
		...
		public static void LoadAllModules()
		{
			...
			IEnumerable<string> allModuleFiles =
				_moduleImplementationDescriptors.Keys.Select(_GetModuleTypeFilter).SelectMany(
					x => System.IO.Directory.GetFiles(Directory, x));
		...
		private static string _GetModuleTypeFilter(ModuleImplementation moduleImplementation)
		{
			...
			moduleImplementation.Path = Directory;
			string moduleTypeFilter = $"Module.{moduleTypeName}.*.dll";
		...
		private static ModuleImplementation _FindImplementation(string type)
		{
			//string directory = Path.GetDirectoryName(filePath);

			string fileName = Path.GetFileNameWithoutExtension(type);
			if (!fileName.StartsWith("Module"))
			{
				return null;
			}

			string moduleType = fileName.Split('.')[1];

			return
				_moduleImplementationDescriptors.Keys.FirstOrDefault(
					x => x.TypeOfModule.Equals(moduleType, StringComparison.OrdinalIgnoreCase));
		}
