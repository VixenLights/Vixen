...
namespace System.Windows.Controls.WpfPropertyGrid
{
    ...
    /// </summary>
    public partial class PropertyGrid : Control, INotifyPropertyChanged, IWeakEventListener
    {
        ...
        {
            if (e is NotifyCollectionChangedEventArgs p)
            {
                Dispatcher.BeginInvoke(new Action(RefreshSelectedObjects));
            }
        ...
        {
            SelectedObjects = ItemsSource.Cast<object>().ToArray();
        }
		...
		public static readonly DependencyProperty ItemSourceProperty =
		    DependencyProperty.Register("ItemSource", typeof(Object), typeof(PropertyGrid),
			    new FrameworkPropertyMetadata(ItemSource_PropertyChanged));


	    private static void ItemSource_PropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
	    {
		    if (d is PropertyGrid p)
		    {
			    p.SelectedObject = p.ItemSource;
		    }
	    }

	    /// <summary>
	    /// A reference to the item that is the source
	    /// Used to bind a singular object to the grid
	    /// </summary>
        ...
        private static void ItemsSource_PropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            if (d is PropertyGrid p)
            {
                p.SelectedObjects = p.ItemsSource.Cast<object>().ToArray();
                if (p.ItemsSource is INotifyCollectionChanged cc)
                {
                    CollectionChangedEventManager.AddListener(cc, p);
                    //cc.CollectionChanged += ItemsSource_CollectionChanged;
                }
            }
        }
        ...
        /// <summary>
        /// A reference to the collection that is the source used
