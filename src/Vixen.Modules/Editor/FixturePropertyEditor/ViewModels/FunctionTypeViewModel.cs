...
namespace VixenModules.Editor.FixturePropertyEditor.ViewModels
{
	...
	public class FunctionTypeViewModel : ItemsViewModel<FunctionItemViewModel>, IFixtureSaveable
	{
		...
		{
			// Initialize the timeline colors from the known colors excluding system colors and transparent
			_timelineColors = ((KnownColor[])Enum.GetValues(typeof(KnownColor)))
				.Select(Color.FromKnownColor)
				.Where(color => color != Color.Transparent && !color.IsSystemColor)
				.ToList();
		}
