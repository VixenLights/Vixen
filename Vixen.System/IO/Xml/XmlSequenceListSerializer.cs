		...
		public IEnumerable<ISequence> ReadObject(XElement element) {
			...
			if(sequencesElement != null) {
				foreach(XElement sequenceElement in sequencesElement.Elements(ELEMENT_SEQUENCE)) {
					...
					VersionedFileSerializer sequenceSerializer = FileService.Instance.CreateSequenceSerializer(fileName);
					ISerializationResult result = sequenceSerializer.Read(fileName);
					ISequence sequence = (ISequence)result.Object;
					...
					//        VersionedFileSerializer<ScriptSequence> scriptSerializer = SerializerFactory.Instance.CreateScriptSequenceSerializer();
