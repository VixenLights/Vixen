...
{
	internal class XmlDisabledControllerCollectionSerializer : IXmlSerializer<IEnumerable<Guid>>
	{
		...
		public IEnumerable<Guid> ReadObject(XElement element)
		{
			...
			if (parentNode != null) {
				ids.AddRange(
					parentNode.Elements().Select(_ReadDisabledDevice).Where(x => x != null).Select(x => x.GetValueOrDefault()));
			}
