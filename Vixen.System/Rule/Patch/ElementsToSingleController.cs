...
namespace Vixen.Rule.Patch
{
	public class ElementsToSingleController : IPatchingRule
	{
		...
		public ElementsToSingleController(IEnumerable<Element> elements, IControllerDevice controllerDevice,
		                                  int startingOutputIndex = 0, int outputsPerElement = 1)
			: this(elements, controllerDevice, controllerDevice.OutputCount, startingOutputIndex, outputsPerElement)
		{
		}

		public ElementsToSingleController(IEnumerable<Element> elements, ISmartControllerDevice controllerDevice,
		                                  int startingOutputIndex = 0, int outputsPerElement = 1)
			: this(elements, controllerDevice, controllerDevice.OutputCount, startingOutputIndex, outputsPerElement)
		{
		}

		public ElementsToSingleController(IEnumerable<Element> elements, IOutputDevice controllerDevice,
		                                  int startingOutputIndex = 0, int outputsPerElement = 1)
			: this(
				elements, controllerDevice, ((IHasOutputs) controllerDevice).OutputCount, startingOutputIndex, outputsPerElement)
		{
		}

		private ElementsToSingleController(IEnumerable<Element> elements, IOutputDevice controllerDevice,
		                                   int controllerOutputCount, int startingOutputIndex, int outputsPerElement)
		{
		...
		{
			return new DataFlowPatch(_controllerOutputComponents[outputIndex].DataFlowComponentId,
			                         _elementComponents[elementIndex].DataFlowComponentId, 0);
		}
