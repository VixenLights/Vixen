...
namespace Vixen.Module.Preview {
	abstract public class FormPreviewModuleInstanceBase : PreviewModuleInstanceBase {
		...
		protected override IThreadBehavior ThreadBehavior {
			get {
				if(_threadBehavior == null) {
					...
					_threadBehavior = VixenSystem.SystemConfig.IsPreviewThreaded ? new MultiThreadBehavior(initMethod) : (IThreadBehavior)new SingleThreadBehavior(initMethod);
				}
				return _threadBehavior;
			}
		}

		abstract protected Form Initialize();
	}
}
