...
{
	internal class SequenceTypeModuleManagement : GenericModuleManagement<ISequenceTypeModuleInstance>
	{
		/// <summary>
		...
		public ISequenceTypeModuleInstance Get(string sequenceFileType)
		{
			...
			ISequenceTypeModuleInstance sequenceType =
				Modules.GetRepository<ISequenceTypeModuleInstance>().GetAll().Cast<ISequenceTypeModuleInstance>().FirstOrDefault(
					x => x.FileExtension.Equals(fileType, StringComparison.OrdinalIgnoreCase));
			return sequenceType;
		}
		...
		public ISequenceTypeModuleInstance GetFactory(ISequence sequence)
		{
			return
				Modules.GetRepository<ISequenceTypeModuleInstance>().GetAll().Cast<ISequenceTypeModuleInstance>().FirstOrDefault(
					x => x.CreateSequence().GetType() == sequence.GetType());
		}
