...
namespace Vixen.Sys {
	class LinearInterpolatedIntentSegmentCollection<TypeOfValue>
		...
		private LinearInterpolatedIntentSegmentCollection(IEnumerable<IntentSegmentNode<TypeOfValue>> segments, ISegmentCreator<TypeOfValue> segmentCreator)
			: this(segmentCreator) {
		...
		public void DivideAt(TimeSpan intentRelativeTime, out LinearInterpolatedIntentSegmentCollection<TypeOfValue> leftSegments, out LinearInterpolatedIntentSegmentCollection<TypeOfValue> rightSegments) {
			FractureAt(intentRelativeTime);
			// Get all segments with an end time <= intentRelativeTime for the left intent.
			leftSegments = new LinearInterpolatedIntentSegmentCollection<TypeOfValue>(_segmentTimeIndex.Values.TakeWhile(x => x.EndTime <= intentRelativeTime), _segmentCreator);
			// Get all segments with a start time >= intentRelativeTime for the right intent.
			rightSegments = new LinearInterpolatedIntentSegmentCollection<TypeOfValue>(_segmentTimeIndex.Values.SkipWhile(x => x.EndTime <= intentRelativeTime), _segmentCreator);
		}
