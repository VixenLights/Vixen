...
{
	internal class ScheduledItemService
	{
		private static ScheduledItemService _instance;
		...
		{
			return new ScheduledItem(scheduledItem.Id, scheduledItem.ItemFilePath, scheduledItem.DayOfWeek,
			                         scheduledItem.StartTime, scheduledItem.RunLength);
		}
		...
		{
			return new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, (int) item.StartTime.TotalHours,
			                    item.StartTime.Minutes, item.StartTime.Seconds);
		}
