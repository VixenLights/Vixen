...
{
	internal partial class DayPanel : UserControl
	{
		...
		private void _DrawTimes(Graphics g)
		{
			...
			for (int y = startHalfHour*HalfHourHeight; y < g.VisibleClipBounds.Bottom && hour < 24; y += HalfHourHeight*2) {
				...
				g.DrawString(hourString, TimeLargeFont, Brushes.Black,
				             midPoint - (int) (g.MeasureString(hourString, TimeLargeFont).Width) + 6, y + 3);
		...
		private void _DrawBlock(ItemBlock block, Graphics g)
		{
			...
			g.DrawString(block.ProgramName, Font, Brushes.Black,
			             new RectangleF(block.Left + 3, block.Top + 2, block.Width - 1, block.Height - 1));
		}

		private void _DrawRoundRect(Graphics g, Pen borderPen, Brush fillBrush, float X, float Y, float width, float height,
		                            float radius)
		{
		...
		private void _CalculateBlocks()
		{
			...
			_layoutEngine.Layout(_blocks,
			                     new Rectangle(TimeGutter, 0, DisplayRectangle.Width - TimeGutter, DisplayRectangle.Height));
		}
