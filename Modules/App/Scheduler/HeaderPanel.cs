...
namespace VixenModules.App.Scheduler {
	public partial class HeaderPanel : UserControl {
		...
		protected virtual void _DrawHeaderButtons(Graphics g, bool hoverLeft, bool hoverRight) {
			...
			if(!g.VisibleClipBounds.IntersectsWith(_buttonLeftBounds) && !g.VisibleClipBounds.IntersectsWith(_buttonRightBounds)) return;
			...
			using(LinearGradientBrush flatGradientBrush = new LinearGradientBrush(new Rectangle(x + 1, y + 1, _buttonWidth - 1, Height), HeaderGradientStart, HeaderGradientEnd, 90),
									  hoverGradientBrush = new LinearGradientBrush(new Rectangle(x + 1, y + 1, _buttonWidth - 1, _buttonWidth - 1), HoverGradientStart, HoverGradientEnd, 90)) {
										  g.FillRectangle(hoverLeft ? hoverGradientBrush : flatGradientBrush, x + 1, y + 1, _buttonWidth - 1, _buttonWidth - 1);
				g.FillRectangle(hoverRight ? hoverGradientBrush : flatGradientBrush, x + 1 + _buttonWidth + _buttonSpace, y + 1, _buttonWidth - 1, _buttonWidth - 1);
			}
