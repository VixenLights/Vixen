...
namespace VixenModules.App.Scheduler {
	class ScheduleService {
		...
		/// <returns></returns>
		public IEnumerable<IScheduleItem> GetItems(IEnumerable<IScheduleItem> items, DateTime startDateTime, DateTime endDateTime) {
		...
		private IEnumerable<IScheduleItem> _GetItemsByDate(IEnumerable<IScheduleItem> items, DateTime startDateTime, DateTime endDateTime) {
		...
		private double _GetWindowEndRepetitions(IScheduleItem scheduleItem, DateTime windowEnd) {
			...
			switch((RecurrenceType)scheduleItem.RecurrenceType) {
				...
				case RecurrenceType.MonthlyDateOfMonth:
					...
					break;
				case RecurrenceType.MonthlyDayOfWeek:
					value = (double)(windowEnd.Month - scheduleItem.StartDate.Month + (windowEnd.Year - scheduleItem.StartDate.Year) * 12) / scheduleItem.DateUnitCount;
					// If the last item occurs after the window, trim it.
