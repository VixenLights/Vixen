...
{
	internal class ProtocolFormatterService
	{
		private static ProtocolFormatterService _instance;
		...
		public IRenardProtocolFormatter FindFormatter(int protocolVersion)
		{
			...
			var formatterTypes =
				assembly.GetTypes().Where(x => x.GetCustomAttributes(typeof (ProtocolVersionAttribute), false).Any());
			Type protocolFormatterImplementation =
				formatterTypes.FirstOrDefault(
					x =>
					(x.GetCustomAttributes(typeof (ProtocolVersionAttribute), false).First() as ProtocolVersionAttribute).Version ==
					protocolVersion);
