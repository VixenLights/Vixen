	...
	public class MovingHeadWPF 
	{
			...
			RenderTargetBitmap rtb)
		{
			...
			if (!_initializedFixtureGeometry)
			{
				...
				InitializeModel(length, maxBeamLength, xOffset, yOffset);
			...
			else
			{
				...
				if (_legendText != MovingHead.Legend ||
        ...
        /// <summary>
		/// Draws the moving head fixture into a bitmap.
		/// </summary>
		...
		public void DrawFixture(int width, int height, double scaleFactor, int xOffset, int yOffset)
        {
			...
			UpdateModel3D(width, height, scaleFactor, xOffset, yOffset, 10, rtb);
