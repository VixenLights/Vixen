...
namespace VixenModules.Preview.DisplayPreview.Model
{
	...
	public class NodeLayout : INotifyPropertyChanged, IHandler<IIntentState<LightingValue>>,
	                          IHandler<IIntentState<CommandValue>>
	{
		...
		{
			get { return VixenSystem.Nodes.GetAllNodes().FirstOrDefault(x => x.Id == NodeId); }
		}
		...
		public void AddColorToNode(Color color)
		{
			...
			SolidColorBrush brush =
				new SolidColorBrush(Color.FromArgb((byte) (intensity*byte.MaxValue), result.ToArgb().R, result.ToArgb().G,
				                                   result.ToArgb().B));
			brush.Freeze();
			SetNodeBrush(brush);
		}
