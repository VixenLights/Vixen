...
namespace CommonElements.ColorManagement.Gradients
{
	...
	public class GradientEdit : Control
	{
		...
		private Gradient _blend;
		private Orientation _orientation = Orientation.Horizontal;
		//selection
		private Gradient.Point _selection = null, _tmp = null;
		...
		/// </summary>
		private Gradient.Point GetFaderUnderMouse(Point location, ref Point offset, out bool focus)
		{
			focus = false;
			if (_blend != null)
			{
				using (Graphics gr = this.CreateGraphics())
				{
					...
					if (_selection != null)
					{
						...
						//focus
						if (HitsSlider(gr, _selection.GetFocusPosition(), _selection is AlphaPoint,
		...
		private bool UpdateSelection(MouseEventArgs e)
		{
			...
			if (_selection != null)
			{
				...
				{
					//move focus point
					_selection.SetFocusPosition(PointToPos(new Point(
							e.X - _offset.X,
							e.Y - _offset.Y)));
				}
		...
		[Browsable(false),
		DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		public Gradient.Point Selection
		{
			...
			set
			{
				...
				if (_selection != null)
				{
					...
					//update focus
					double foc = _selection.GetFocusPosition();
					if (!double.IsNaN(foc))
				...
				if (value != null)
				{
					...
					//update focus
					double foc = value.GetFocusPosition();
					if (!double.IsNaN(foc))
						this.Invalidate(PosToRectangle(foc));
				}
				_selection = value;
